...
# %%
images = np.zeros((len(class_numers_things),25, 1,257,1024))
...
for i, row in tqdm(data_things.iterrows(), total=len(data_things)):
    img_1 = images[row.Im1] # dim = 25,1,257,1024
...
for k in range(len(correct)):
    print(f"CLIP large14 THINGS-odd-one-out accuracy layer {k}= {100*correct[k]/total:.3f}")
