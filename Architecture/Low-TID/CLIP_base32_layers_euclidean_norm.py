...
model = CLIPVisionModel.from_pretrained("openai/clip-vit-base-patch32").to(device)     # if model = base -> layers = 13; if model = large -> layers = 25
...
distances_2013_clip = np.zeros((13,3000,768))
...
distances_2013_clip_norm = np.zeros((13,3000,768))
...
for i in range(len(features_img)):
    print(f'CLIP base32 TID-2013 spearman corr layer {i} = {-spearmanr(np.sqrt(distances_2013_clip_norm.mean(axis=2))[i],data_tid_2013.MOS)[0]:.3f}')
