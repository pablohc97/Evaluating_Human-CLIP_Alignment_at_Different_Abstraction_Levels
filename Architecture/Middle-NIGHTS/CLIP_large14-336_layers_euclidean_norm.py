...
model = CLIPVisionModel.from_pretrained("openai/clip-vit-large-patch14-336").to(device)     # if model = base -> layers = 13; if model = large -> layers = 25
...
d1 = np.zeros((25,20019,1024))
d2 = np.zeros((25,20019,1024))
...
d1_norm = np.zeros((25,20019,1024))
d2_norm = np.zeros((25,20019,1024))
...
for k in range(len(correct)):
    print(f"CLIP large14-336 NightsPerceptualDataset accuracy layer {k} = {100*correct[k]/total:.3f}")