...
for i, row in tqdm(data_things.iterrows(), total=len(data_things)):
    img_1 = images[row.Im1] # dim = 13,1,197,768
    img_2 = images[row.Im2]
    img_3 = images[row.Im3]

    d_12_i = np.mean((img_1 - img_2)**2, axis=(1,2)) # dim = 13,768
...
d_12_means = d_12.mean(axis=1)  # dim = 13,768
...
for i in range(13):
    for j in range(768):
        d_12_norm[i,:,j] = d_12[i,:,j] / d_means[i,j]    #/ d_12_means[i,j]
        d_13_norm[i,:,j] = d_13[i,:,j] / d_means[i,j]    #/ d_13_means[i,j]
        d_23_norm[i,:,j] = d_23[i,:,j] / d_means[i,j]    #/ d_23_means[i,j]


d_12_norm_fin = np.sqrt(d_12_norm.mean(axis=2))  # dim = 13,num_img
...
for k in range(len(correct)):
    print(f"CLIP base16 THINGS-odd-one-out accuracy layer {k} euclidean norm = {100*correct[k]/total:.3f}")
