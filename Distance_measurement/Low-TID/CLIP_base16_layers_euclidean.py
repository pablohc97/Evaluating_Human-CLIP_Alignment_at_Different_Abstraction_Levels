...
distances_2013_clip = np.zeros((3000,13))
...
for i, row in tqdm(data_tid_2013.iterrows(), total=3000):
    ...
    for j in range(len(features_img)):
        d = np.sqrt(np.mean((features_img[j].cpu().numpy() - features_dist_img[j].cpu().numpy())**2))
        distances_2013_clip[i,j] = d

for k in range(len(features_img)):
    data_tid_2013[f'Distance_CLIP_{k}'] = distances_2013_clip[:,k]

for k in range(len(features_img)):
    pearson = pearsonr(data_tid_2013.MOS, data_tid_2013[f'Distance_CLIP_{k}'])[0]
    spearman = spearmanr(data_tid_2013.MOS, data_tid_2013[f'Distance_CLIP_{k}'])[0]
    print(f"CLIP base16 corr in TID-2013 layer {k} euclidean: Pearson = {-pearson:.3f}     Spearman = {-spearman:.3f}")