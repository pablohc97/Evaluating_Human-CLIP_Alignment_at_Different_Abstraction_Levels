...
for i, row in tqdm(data_tid_2013.iterrows(), total=3000):
    ...
    img_inputs = processor(images=img, text=[''], padding=True, return_tensors="pt").to(device)
    dist_img_inputs = processor(images=dist_img, text=[''], padding=True, return_tensors="pt").to(device)
...
print(f'ChineseCLIP base16 TID-2013 spearman corr projection euclidean norm = {-spearmanr(np.sqrt(distances_2013_clip_norm.mean(axis=1)),data_tid_2013.MOS)[0]:.3f}')