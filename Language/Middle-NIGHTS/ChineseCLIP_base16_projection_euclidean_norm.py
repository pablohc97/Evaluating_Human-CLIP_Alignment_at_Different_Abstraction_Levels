...
for i, row in tqdm(data_night.iterrows(), total=20019):
    ...
    input_img = processor(images=img, text=[''], padding=True, return_tensors="pt").to(device)
    input_dist_img_1 = processor(images=dist_img_1, text=[''], padding=True, return_tensors="pt").to(device)
    input_dist_img_2 = processor(images=dist_img_2, text=[''], padding=True, return_tensors="pt").to(device)
...
print(f"ChineseCLIP base16 NightsPerceptualDataset accuracy projection euclidean norm = {100*correct/total:.3f}")