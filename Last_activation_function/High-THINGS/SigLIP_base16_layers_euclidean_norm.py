...
model = SiglipVisionModel.from_pretrained("google/siglip-base-patch16-224").to(device)     # if model = base -> layers = 13; if model = large -> layers = 25
...
for k in range(len(correct)):
    print(f"SigLIP base16 THINGS-odd-one-out accuracy layer {k} euclidean norm = {100*correct[k]/total:.3f}")
