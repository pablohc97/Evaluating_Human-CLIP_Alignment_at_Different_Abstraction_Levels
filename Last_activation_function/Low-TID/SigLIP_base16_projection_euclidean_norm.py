...
distances_2013_clip = np.zeros((3000,768))
...
for i, row in tqdm(data_tid_2013.iterrows(), total=3000):
    ...
    img_inputs = processor(images=img, text=[''], padding="max_length", return_tensors="pt").to(device)
    dist_img_inputs = processor(images=dist_img, text=[''], padding="max_length", return_tensors="pt").to(device)
...
distances_2013_clip_norm = np.zeros((3000,768))
...
print(f'SigLIP base16 TID-2013 spearman corr projection euclidean norm = {-spearmanr(np.sqrt(distances_2013_clip_norm.mean(axis=1)),data_tid_2013.MOS)[0]:.3f}')