...
distances1_clip = np.zeros((20019,768))
distances2_clip = np.zeros((20019,768))
...
for i, row in tqdm(data_night.iterrows(), total=20019):
    ...
    input_img = processor(images=img, text=[''], padding="max_length", return_tensors="pt").to(device)
    input_dist_img_1 = processor(images=dist_img_1, text=[''], padding="max_length", return_tensors="pt").to(device)
    input_dist_img_2 = processor(images=dist_img_2, text=[''], padding="max_length", return_tensors="pt").to(device)
...
distances1_clip_norm = np.zeros((20019,768))
distances2_clip_norm = np.zeros((20019,768))
...
print(f"SigLIP base16 NightsPerceptualDataset accuracy projection euclidean norm = {100*correct/total:.3f}")